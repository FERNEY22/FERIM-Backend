...
// Controlador para crear una nueva solicitud de reserva: POST /api/reservations
...
// Controlador para obtener todas las reservas de un inquilino: GET /api/reservations/tenant
// Requiere autenticación
exports.getTenantReservations = async (req, res) => {
  try {
    // Verificar que el usuario autenticado es un inquilino
    const user = await User.findById(req.user.id).select('-password');
    if (!user || user.role !== 'inquilino') {
...
// Controlador para obtener todas las reservas de propiedades de un propietario: GET /api/reservations/owner
// Requiere autenticación
...
// Controlador para actualizar el estado de una reserva (Aceptar/Rechazar): PUT /api/reservations/:id
...
//new added 14-12-25
// Controlador para validar si un email pertenece a un inquilino registrado
exports.validateTenant = async (req, res) => {
  try {
    ...
    if (user.role !== 'inquilino') {
      return res.status(403).json({ valid: false, message: 'Solo inquilinos pueden hacer reservas.' });
    }
